...
import { convertToClassificationType } from '../../utils';
...
export const ProductsContextProvider = ({ children }: Props) => {
  ...
  const classificationSelected = convertToClassificationType(searchParams.get('q') ?? 'ALL');
  ...
  const changeClassificationFilter = (e: React.ChangeEvent<HTMLSelectElement>) => {
    setSearchParams((prev) => {
      prev.set('q', convertToClassificationType(e.currentTarget.value));
      prev.set('page', '0');
      return prev;
    });
  };
